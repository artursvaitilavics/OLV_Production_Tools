...
class NA_OT_AlignNodes(Operator):
    ...
    margin: IntProperty(name='Margin', default=50,
    ...
    def execute(self, context):
        ...
        if horizontal:
            selection = sorted(
                selection, key=lambda n: n.location.x + (n.dimensions.x / 2))
        else:
            selection = sorted(
                selection, key=lambda n: n.location.y - (n.dimensions.y / 2), reverse=True)
